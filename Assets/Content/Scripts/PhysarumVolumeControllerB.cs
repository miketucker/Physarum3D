...
public class PhysarumVolumeControllerB : MonoBehaviour
{
    //DONT CHANGE THESE
    const int READ = 0;
    const int WRITE = 1;
    ...
    [Range(0f, 1f)] public float respawnProbability = 0;
    [Range(0f, 0.5f)] public float spawnRadius = 0.1f;
    ...
    [Range(0f, 1f)] public float centralPull = 0;
    ...
    public float feedStrength = 0f;
    public float feedDistance = 0.1f;
    ...
    private static int groupCount1D = 32;
    ...
    void UpdateRuntimeParameters()
    {
        ...
        shader.SetFloat("_RespawnProbability", respawnProbability);
        shader.SetFloat("_SpawnRadius", spawnRadius);
        shader.SetFloat("_CentralPull", centralPull);
        if (feeder != null){
			Vector3 feedPos = feeder.position;
			Vector4 feed = new Vector4();
			feed.x = feedPos.x;
			feed.y = feedPos.y;
			feed.z = feedPos.z;
			feed.w = feedStrength;
			
			shader.SetVector("_Feeder", feed);
			shader.SetFloat("_FeedDistance", feedDistance);
		}
    }
